...
# Move stores (x,y,side) i.e number off missionaries,
# cannibals to be move from left to right or right to left for particular state
...
class Solution():
    ...
    def __init__ (self):
        ...
        self.graph = pydot.Dot(graph_type='graph', bgcolor='#fff3af',
                               label= "fig: Missionaries and Cannibal State Space Tree", fontcolor="red", fronsize="24")
        self.visited={}
        self.solved = False

    def is_valid_move(self, number_missionaries, number_cannibals):
        """
        Checks if number constraints are sastified
        """
    ...
    def draw_legend(self):
        ...
        node5 = pydot.Node("5", style="filled", fillcolor="green", label ="Goal nodes", width="2", fixedsize="true")
    ...
    def draw(self, *, number_missionaries_left, number_cannibals_left, number_missionaries_right, number_cannibals_right):
        ...
        left_c=emoji.emojize(f":orge: " * number_cannibals_left)
        right_m=emoji.emojize(f":old_man" * number_missionaries_right)
        right_c=emoji.emojize(f":orge: " * number_cannibals_right)
    ...
    def show_solution(self):
        ...
        # And find parent untill start state is reached
